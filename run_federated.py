...
from run_local_and_centralized import get_window_accuracy_on_dataset
from utils import build_dataset
from settings import NUMBER_OF_ACTIVITIES, NUM_ROUNDS, BATCH_SIZE, BUFFER_SIZE, USE_DAY, USE_TIME, \
    HISTORY_SIZE, TOTAL_NUM_OF_FEATURES, LIST_OF_DAYS_OF_UPDATES, AVAILABLE_CLIENTS, CLIENT_START_DAY
...
def build_meta_fed_train_dataset(current_days, data_keys):
    ...
    for i in AVAILABLE_CLIENTS[data_keys]:
        num_days = current_days - CLIENT_START_DAY[i] + 1
        if num_days > 0:
...
logdir = "./logs_fl_wd_avg_sampling/"
